  ...
  <section class="sidebar">
    ...
    <ul>
      <li
        ...
        :key="list.list_id"
        @click="
          setTodos(list.list_ID),
            setCurrentList(list.list_ID),
            getCompleted(list.list_ID)
        "
        class="notActive"
        :class="{ isActive: list.name === $store.state.currentList.name }"
      >
        {{ list.name }}
...
export default {
  ...
  methods: {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          const response = await TodoService.getTodos(
            this.$store.state.lists[0].list_ID
          );
          this.$store.commit("SET_TODOS", response.data);
          const response2 = await TodoService.getCompleted(
            this.$store.state.lists[0].list_ID
          );
    ...
    createList() {
      ListService.createList(this.newList).then((response) => {
        if (response.status === 200) {
    ...
    deleteList(id) {
      ListService.deleteList(id).then((response) => {
        if (response.status === 200) {
          this.getLists(this.$store.state.user.userId);
          this.setTodos(
            this.$store.state.lists.length
              ? this.$store.state.lists.length - 1
              : this.$store.state.lists[0].list_ID
          );
          this.setCurrentList(
            this.$store.state.lists.length
